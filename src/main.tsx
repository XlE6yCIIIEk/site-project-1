...
function Navbar() {
  ...
  return <header className={scrolled ? 'nav is-scrolled' : 'nav'}><div className="nav-inner"><a className="brand" href="#top" onClick={event => handleNavigation(event, 'top')}><span>poryadok_s_irinoi</span><small>ПЕРЕЕЗД ПОД КЛЮЧ</small></a><nav className={open ? 'nav-links is-open' : 'nav-links'}>{nav.map(([label,id]) => <a className={activeId === id ? 'is-active' : ''} key={id} href={`#${id}`} onClick={event => handleNavigation(event, id)}>{label}</a>)}</nav><a className="wa-nav" href={whatsappUrl} target="_blank" rel="noreferrer"><Icon name="whatsapp" size={18}/><span>WhatsApp</span></a><button className="menu" aria-label={open ? 'Закрыть меню' : 'Открыть меню'} aria-expanded={open} onClick={() => setOpen(!open)}><Icon name={open ? 'close' : 'menu'}/></button></div></header> }
