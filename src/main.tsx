...
const reviews = [
  ...
  ['Марина Т.', 'Переезд офиса и личных вещей', 'Команда спокойно решила сложную задачу за три дня: мебель разобрали, перевезли и собрали. Все этапы прошли по понятному плану и без лишней суеты.', 'МТ']
] as const
const faqItems = [
  ['Сколько стоит переезд?', 'Стоимость рассчитывается индивидуально после знакомства с вашим пространством. Она зависит от квадратуры, объёма вещей, состава услуг, маршрута, этажности и времени, которое понадобится команде.'],
...
function Navbar() {
  ...
  return <header className={scrolled ? 'nav is-scrolled' : 'nav'}><div className="nav-inner"><a className="brand" href="#top" onClick={event => handleNavigation(event, 'top')}><span>poryadok_s_irinoi</span><small>ПЕРЕЕЗД ПОД КЛЮЧ</small></a><nav className={open ? 'nav-links is-open' : 'nav-links'}>{nav.map(([label,id]) => <a className={activeId === id ? 'is-active' : ''} key={id} href={`#${id}`} onClick={event => handleNavigation(event, id)}>{label}</a>)}</nav><a className="instagram-nav" href={INSTAGRAM_URL} target="_blank" rel="noreferrer" aria-label="Instagram"><Icon name="instagram" size={18}/></a><a className="wa-nav" href={whatsappUrl} target="_blank" rel="noreferrer"><Icon name="whatsapp" size={18}/><span>WhatsApp</span></a><button className="menu" aria-label={open ? 'Закрыть меню' : 'Открыть меню'} aria-expanded={open} onClick={() => setOpen(!open)}><Icon name={open ? 'close' : 'menu'}/></button></div></header> }
...
<section className="section result"><div className="container result-wrap"><img src="/images/calm-home.jpg" alt="Готовый к жизни дом после переезда" className="result-image reveal reveal-scale site-image" loading="lazy"/><div className="result-copy reveal reveal-right"><p className="eyebrow">ПОСЛЕ ПЕРЕЕЗДА</p><h2>Вы получаете не просто переезд.<em>А готовый к жизни дом.</em></h2><ul>{[['Вещи уже рассортированы','layers'],['Мебель собрана','chair'],['Всё необходимое на своих местах','home'],['Вам остаётся только начать жить','heart']].map(([title, icon], index) => <li className="reveal reveal-up result-item" style={{ transitionDelay: `${120 + index * 75}ms` }} key={title}><Icon name={icon as IconName}/>{title}</li>)}</ul><p className="result-note">Вы приезжаете — а не разбираете горы коробок.</p></div></div></section>
...
<section className="section emotional"><div className="container emotional-wrap"><div className="emotional-copy reveal reveal-left"><p className="eyebrow">ВАШ НОВЫЙ ДОМ</p><h2>Представьте свой переезд</h2><p>Не горы коробок.<br/>Не поиски зарядки среди десяти пакетов.<br/>Не вопрос: «А где вообще лежат полотенца?»</p><strong>А вы открываете шкаф — и всё необходимое уже на месте.</strong><small>Именно такой переезд мы хотим для вас организовать.</small></div><img src="/images/organized-home.jpg" alt="Организованное пространство нового дома" className="emotion-image reveal reveal-scale site-image" loading="lazy"/></div></section>
...
<FAQ/></main><footer><div className="container footer-row"><a className="brand" href="#top"><span>poryadok_s_irinoi</span><small>ПЕРЕЕЗД ПОД КЛЮЧ</small></a><p>© 2026 Переезд под ключ. Все права защищены.</p><button type="button" className="privacy-link" onClick={() => setConsentDialogOpen(true)}>Конфиденциальность</button><div className="footer-socials"><a href={INSTAGRAM_URL} target="_blank" rel="noreferrer"><Icon name="instagram"/> Instagram</a><a href={whatsappUrl} target="_blank" rel="noreferrer"><Icon name="whatsapp"/> +7 707 773 1752</a></div></div></footer></> }
